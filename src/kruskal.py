...
def kruskal(g):
    ...
    # Verifie si il y a des arets
    if len(edges) == 0:
        return 1
    
    # Ajoute le première edge incondicionellement
    tree.addEdge(g.nodes[edges[0].id1], g.nodes[edges[0].id2], edges[0].weight)
    ...
    for edge in edges[1:]:
        if (tree.createACycle(edge) == False):
            tree.addEdge(g.nodes[edge.id1], g.nodes[edge.id2], edge.weight)
            addedEdges += 1
